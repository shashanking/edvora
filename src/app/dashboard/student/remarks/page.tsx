...
function TypeBadge({ type }: { type: string }) {
  ...
  return (
    <span
      className={`px-2.5 py-1 rounded-full text-xs font-medium capitalize ${styles[type] || "bg-gray-100 text-gray-600"}`}
    >
      {type}
    </span>
  );
...
export default async function StudentRemarksPage() {
  ...
  // Fetch remarks for the logged-in student
  ...
  const rows = (remarksData as unknown as RemarkRow[]) || [];

  // Gather unique course and teacher IDs
  const courseIds = [...new Set(rows.map((r) => r.course_id))];
  const teacherIds = [...new Set(rows.map((r) => r.teacher_id))];

  // Fetch related courses and teacher profiles
  ...
  const teacherMap = new Map(
    ((teachers as { id: string; full_name: string }[]) || []).map((t) => [t.id, t.full_name])
  );
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-[#4D4D4D] text-sm mt-1">
          View feedback shared by your teachers
        </p>
      </div>
            ...
            >
              <div className="flex items-start justify-between gap-4">
                <div className="min-w-0 flex-1">
                  ...
                  <p className="text-sm text-[#1C1C28] leading-relaxed">
                    {r.content}
                  </p>
                </div>
                <div className="flex items-center gap-2 flex-shrink-0">
                  <div className="w-9 h-9 bg-[#1F4FD8] rounded-full flex items-center justify-center">
                    <span className="text-white text-sm font-bold">
                      {r.teacher_name?.charAt(0)?.toUpperCase() || "T"}
                    </span>
                  </div>
