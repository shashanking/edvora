...
const CourseCard = ({ item }: { item: LandingCourseItem }) => (
  <div className="bg-white/95 rounded-[24px] border-2 border-[#ffc83d5c] p-5 flex flex-col gap-4 w-full max-w-[501px] mx-auto hover:shadow-2xl transition-all duration-300 hover:-translate-y-2 shadow-[0_20px_60px_rgba(224,168,0,0.18)]">
    {/* Image */}
    <div className="w-full h-[240px] bg-[#FFF3C4] rounded-[12px] overflow-hidden relative">
      <img src={item.image} alt={item.program} className="w-full h-full object-cover" />
    </div>

    {/* Text Content */}
    <div className="flex flex-col gap-4 flex-1">
      <h3 className="text-[#8A6500] font-poppins font-semibold text-[20px] md:text-[24px] leading-[30px]">{item.program}</h3>
      <p className="text-[#5F4A00] font-['Nunito_Sans'] font-normal text-[16px] md:text-[18px] leading-[25px]">{item.gain}</p>

      {/* Duration & Rating */}
      <div className="flex justify-between items-start">
        <div className="flex flex-col gap-1">
          <span className="text-[#8A6500] font-poppins font-normal text-[12px] leading-[18px]">Duration</span>
          <span className="text-[#6E5200] font-poppins font-semibold text-[20px] leading-[30px]">{item.duration}</span>
        </div>
        <div className="flex flex-col gap-1">
          <span className="text-[#8A6500] font-poppins font-normal text-[12px] leading-[18px]">Rating</span>
    ...
    {/* CTA */}
    <a href="#contact" className="w-full h-[50px] bg-[#8A6500] hover:bg-[#6E5200] text-white font-poppins font-semibold text-[20px] leading-[30px] rounded-[66px] transition-all duration-300 flex items-center justify-center shadow-lg shadow-yellow-900/20">
      Enroll Now!
    </a>
  </div>
);
...
const AdultLearnersSection: React.FC<AdultLearnersSectionProps> = ({ coursesByCategory }) => {
  ...
  return (
    <section id="adult-learners" className="relative w-full min-h-screen bg-[#FFC83D] py-12 md:py-16 lg:py-20 px-4 md:px-6 lg:px-12 overflow-hidden">
      ...
      <div className="max-w-[1400px] mx-auto relative z-10">
        ...
        <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-6 md:gap-8 mb-8 md:mb-12 py-3">
          ...
          <div>
            ...
            <p className="text-[#5A4300] font-poppins font-semibold text-[14px] sm:text-[16px] md:text-[20px] lg:text-[28px] leading-tight tracking-normal mb-2">(Ages 16+)</p>
            <p className="text-[#6E5200] font-nunito font-semibold text-[12px] sm:text-[14px] md:text-[16px] lg:text-[20px] leading-relaxed tracking-normal">
              ...
              <button
                ...
                className={`px-3 sm:px-4 md:px-6 py-2 sm:py-2.5 rounded-full transition-all duration-300 font-nunito font-semibold text-[9px] sm:text-[10px] md:text-[14px] lg:text-[16px] leading-tight tracking-normal ${
                  selectedCategory === "professional"
                    ? "bg-[#1F4FD8] text-white shadow-lg shadow-blue-900/20"
                    : "bg-white/20 text-[#2B2B2B] hover:bg-white/30 border border-[#E0A800]/30 backdrop-blur"
                }`}
              >
        ...
        <div className="flex justify-center w-full">
          <a href="#contact" className="w-full md:w-auto px-12 py-4 bg-[#1F4FD8] hover:bg-[#1A45C2] text-white text-lg rounded-full transition-all duration-300 shadow-lg hover:shadow-xl hover:scale-105 font-poppins font-semibold text-[20px] leading-none tracking-normal text-center inline-block">
            Book Free Trial
          </a>
        </div>
      </div>
